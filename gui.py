...
class ArchiveOrganiserApp(ctk.CTk):
    def __init__(self) -> None:
        ...
        self._saved_layout_ids = [
            str(x) for x in (self._settings.get("layout_ids") or []) if str(x).strip()
        ]
    ...
    def _build_layout(self) -> None:
        ...
        # Restore destination / layout ticks from disk after widgets exist
        self._apply_saved_organise_preferences()
    ...
    def _apply_saved_organise_preferences(self) -> None:
        """Restore destination and layout ticks from saved settings."""
        saved_dest = str(self._settings.get("destination") or "")
        if saved_dest and hasattr(self, "dest_entry"):
...
4. Organise tab
   ...
   • Layout choices and advanced knobs are remembered next launch.
...
""",
    ...
    def preview_organise(self) -> None:
        ...
        lines = [
            ...
            f"Media date: {options.media_date_depth} · Docs by extension: {options.documents_by_ext} · Separate archives: {options.separate_archives}",
    ...
    def _organise_plan_key(self, dest: str, layout_ids, options: OrganiseOptions) -> str:
        ...
        modes = ",".join(
            ...
            if v and v != "layout_default"
        )
        return (
            f"{dest}|{layouts}|{cats}|{options.copy_instead_of_move}|"
            f"{options.media_date_depth}|{options.documents_by_ext}|"
            f"{options.separate_archives}|{modes}|"
